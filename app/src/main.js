...
const App = {
    ...
    register: async function(user) {
        ...
        const selectedpatient = document.querySelector("input[name='inpatient']:checked");
        const inpatient=(selectedpatient) ? selectedpatient.value : null;
        ...
        if (checkedValues) {
            ...
            if (!validate) {        
                console.log(fullname, age, gender, medical_id, blood_type, organ, weight, height,inpatient);
                if (user=="Donor")
                    this.setDonor(fullname, age, gender, medical_id, blood_type, organ, weight, height);
                else if (user=="Patient") 
                    this.setPatient(fullname, age, gender, medical_id, blood_type, organ, weight, height,inpatient);
    ...
    setPatient: async function(fullname, age, gender, medical_id, blood_type, organ, weight, height,inpatient) {

        const gas = await this.contractInstance.methods.setPatients(fullname, age, gender, medical_id, blood_type, organ, weight, height,inpatient).estimateGas({
            from: this.accounts[0]
        });
        await this.contractInstance.methods.setPatients(fullname, age, gender, medical_id, blood_type, organ, weight, height,inpatient).send({
            from: this.accounts[0], gas: Math.max(gas, MIN_GAS)
        });
    ...
    viewPatients: async function() {
        ...
        for (let i=0; i<patientCount; i++) {
            await this.contractInstance.methods.getPatient(patientIDs[i]).call().then(function(result) {
                console.log(result);
                patient = [
                    { Index: i+1, "Full Name": result[0], Age: result[1], Gender: result[2], "Medical ID": patientIDs[i], "Blood Type": result[4], "Organ(s)": result[5], "Weight(kg)": result[6], "Height(cm)": result[7],"Inpatient":result[8]},
                ];
    ...
    transplantMatch: async function() {
        ...
        for (var i=0; i<patientCount; i++) {
            ...
            await this.contractInstance.methods.getPatient(patientIDs[i]).call().then(function(result){
                ...
                patientbloodtype=result[4];
                patientorgans=result[5];
            });
